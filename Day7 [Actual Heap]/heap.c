...
static void *HeapEnd  = &Heap_arr[HEAP_SIZE-1];
        ...
        // now we will use the actual heap so we will use the system call get the start of the heap
	// i will set this variables to NULL and at the first call of malloc i will assign them
	// i will know that it is the first time by "times" var
	int times = 0;
...
printf("Prog_Brk(initially) = %p \n",Prog_Brk);
		...
		// if it returns this value so the heap is full so there is afree space at it
#if(HEAP_KIND == ACTUAL_HEAP)
	      ...
	      // Prog_Brk = 
	//      sbrk(((int)((size+ALL_METADATA)/PAGE_SIZE)+1)*PAGE_SIZE);
	      Prog_Brk = sbrk(0);
#endif
...
{
	// the HmmCalloc allocates array with speciefic length and size of each element on the array
	// then it will set all the elements of the array to zero
	// "size" variable represents the size of each element on the allocated array ex: sizeof(int)
	// "nmemb" variable represents the number of elements on this array
	void *ptr = NULL;
...
// we have 4 cases
// (1) ptr  = NULL & size != 0 >>> the HmmRealloc will act as HmmAlloc function
// (2) ptr != NULL & size  = 0 >>> the HmmRealloc will act as HmmFree function
// (3) ptr == NULL & size == 0 >>> Not defined behavior so the returned = NULL
// (4) ptr != NULL & size != 0 >>> we have steps on this case
