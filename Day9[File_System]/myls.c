...
void do_simple_ls(int argc,char **argv){
   ...
   /* this if handles logic on -d option */
   if(ON == Enable_List_Dir){
   	printf("\n");
   }
	   ...
	   for(int i = 0;i<FilesNum;i++){
		 /* if the hidden files option is enabled so it will be printed */
			...
			if(ON == Enable_Long_Format){
				...
				printf("%s",  ArrayOfInfo[i].FileInfo.d_name);
...
void vdPrintPermissions(int  mode){
   ...
   else if ((mode &  0170000) == 0020000)	str[0] = 'c';
   else if ((mode &  0170000) == 0040000)	str[0] = 'd';
   else if ((mode &  0170000) == 0060000)	str[0] = 'b';
   else if ((mode &  0170000) == 0100000)	str[0] = '-';
   else if ((mode &  0170000) == 0120000)	{str[0] = 'l'; SymLinkFlag = ON;}
   else if ((mode &  0170000) == 0140000)	str[0] = 's';
